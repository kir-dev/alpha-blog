...
export class AuthSchStrategy extends PassportStrategy(Strategy) {
  constructor(private authService: AuthService) {
    super({
      ...
      scopes: [AuthSchScope.PROFILE, AuthSchScope.EMAIL, AuthSchScope.PEK_PROFILE],
    });
  }

  async validate(userProfile: AuthSchProfile): Promise<{ name: string; email: string; id: number }> {
